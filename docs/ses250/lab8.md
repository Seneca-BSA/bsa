...
![Figure 8.3 Keysight 33500B Function Generator](lab8-fg.png)

***Figure 8.3** Keysight 33500B Function Generator*
...
![Figure 8.4 Keysight DSO-X 2002A Digital Storage Oscilloscope](lab8-dso.png)

***Figure 8.4** Keysight 33500B Function Generator*
...
1. Turn on the oscilloscope with the power button (8.5-1) if it is not already on.
1. Identify the two cables with a BNC connector beside the oscilloscope. Connect the BNC cables to the CH1 (8.5-4) and CH2 connectors of the oscilloscope as shown.

    ![Figure 8.6](lab8-dso-connection-1.png)

    ***Figure 8.6***

1. Press the Auto Scale button (8.5-9). If the Auto Scale function is disabled, skip this step.
1. Press the **CH1** On/Off button (8.5-6) so CH1 is on (button lights up green).
1. Press the **CH2** On/Off button so CH2 is off (no light showing at the button). The bright lines on the DSO display the waveform of the signal as a function of time. When there is no signal, the waveform is a flat line. If you don't see a flat line, that means your DSO is displaying amplified electronic noise from the environment. Don't worry if you don't see a flat line for this step of the lab.
    ...
    > **Lab Question 1:** How many signal line(s) (8.5-13, bright solid horizontal line) do you see on the display? What is the colour of the line(s)?

1. Press the **CH2** On/Off button to turn CH2 on (button lights up green) then press the Auto Scale button (8.5-9) again.

    > **Lab Question 2:** How many signal line(s) do you see on the display? What is the colour of the line(s)?

1. You can adjust the vertical position of those signal lines for measurement or display purposes. Turn the CH1 vertical position knob (8.5-5) until the CH1 signal line is reasonably lined up vertically centred on the display.
...
1. Identify the Function Generator at your workbench. Disconnect any cable plug into it so it is as shown below.
    ...
    ![Figure 8.7](lab8-fg-1.png)

    ***Figure 8.7***

1. Identify the cable with a BNC connector beside the function generator and connect it to the Output of the function generator (8.7-3). Afterward, push the power button (8.7-1) to turn on the function generator.
1. After the function generator is turned on, we'll set up a 1kHz Sine wave, press the Waveforms button (8.7-7) and select Sine waveform mode from button menus (8.7-2).
1. Then press the Parameters button (8.7-8) and use the keypad, digit selector buttons (8.7-5), and the adjustment knob (8.7-6) to set the following:
...
1. Identify the cable connected to the function generator and the alligator clips at the end of the cable connected to **CH1** of the oscilloscope. Connect the two red alligator clips together and connect the two black alligator clips together.
    ...
    ![Figure 8.8](lab8-fg-connection-1.png)

    ***Figure 8.8***

1. Once the cables are connected and the waveform is set, push the channel button on the function generator (8.9-4) then turn the output to “On” (8.9-9).

    ![Figure 8.9](lab8-fg-2.png)

    ***Figure 8.9***

1. You should now see some signal on the DSO. If not, push the Auto Scale button on the oscilloscope (8.5-9). You should see a waveform that is similar to that shown in the figure below.

    ![Figure 8.10](lab8-dso-2.png)

    ***Figure 8.10***

1. Turn the time division knob (8.5-12) and the horizontal position knob (8.5-11) a few ticks clockwise then counter-clockwise.

    > **Lab Question 3:** What effect do the time division knob and the horizontal position knob have on the waveform?

1. The grey-white vertical lines (8.10-2) on the oscilloscope chart are called time division lines and the top right of the display shows the time divisor (8.10-4). In the figure above, the time divisor is set to 200.0μs/ which means each time division line represents 200.0μs in the time domain. The are a total of 10 time divisions in the display.
...
1. The grey-white horizontal line (8.10-1) on the oscilloscope display is called a voltage division line and at the top left of the display shows the voltage divisor (8.10-3). In the figure above, 50.0mV/ means each line represents 50.0mV in the voltage range. The are a total of 8 voltage divisions in the display.
...
1. Afterward, press the Output Load button (8.9-10) and change the setting to "High-Z".
...
1. You should now see a 3 V peak-to-peak signal on the oscilloscope. Adjust the voltage division knob (8.5-7) so the sine wave will fit into the screen again.
    ...
    **NOTE:** Press the CH1 button (8.5-6) and ensure the “Coupling” setting on the oscilloscope at the lower left corner of the display is set as “DC” and the “Probe” setting at the lower right corner is selected as “1X” If not, correct them by pressing and selecting the respective settings and change them with the selector knob (8.5-3). The knob can be turned and pressed.
